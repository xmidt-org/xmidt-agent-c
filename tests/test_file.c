...
void test_fslirp(void)
{
    ...
    CU_ASSERT_FATAL(0 == fslirp(filename, 0, (void **)&data, &len));
    ...
    CU_ASSERT_FATAL(0 == fslirp(filename, 100, (void **)&data, &len));
    CU_ASSERT_FATAL(NULL != data);
    CU_ASSERT(strlen(text) == len);
    CU_ASSERT_NSTRING_EQUAL(text, data, strlen(text));
    free(data);
    ...
    CU_ASSERT_FATAL(0 == fslirp(filename, strlen(text), (void **)&data, &len));
    CU_ASSERT_FATAL(NULL != data);
    CU_ASSERT(strlen(text) == len);
    CU_ASSERT_NSTRING_EQUAL(text, data, strlen(text));
    free(data);
    ...
    CU_ASSERT_FATAL(E2BIG == fslirp(filename, 5, (void **)&data, &len));
    CU_ASSERT_FATAL(NULL == data);
    CU_ASSERT(0 == len);

    CU_ASSERT_FATAL(EINVAL == fslirp("__invalid__", 0, (void **)&data, &len));
...
void add_suites(CU_pSuite *suite)
{
    *suite = CU_add_suite("utils.c tests", NULL, NULL);
    CU_add_test(*suite, "fslirp() Tests", test_fslirp);
}
